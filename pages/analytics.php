...
if($conn){
    ...
    $r=$conn->query("SELECT COUNT(*) c FROM thfp_bouts WHERE bout_type='final' AND winner_id IS NOT NULL AND result_method!='no_contest'");$gc=$r?((int)$r->fetch_assoc()['c']):0;
    ...
    $medals=$mr?$mr->fetch_all(MYSQLI_ASSOC):[];
    ...
    $mrows=$mres?$mres->fetch_all(MYSQLI_ASSOC):[];$total_m=array_sum(array_column($mrows,'cnt'));
    ...
    $top=$tres?$tres->fetch_all(MYSQLI_ASSOC):[];
    ...
    $gym_rows=$gres?$gres->fetch_all(MYSQLI_ASSOC):[];
