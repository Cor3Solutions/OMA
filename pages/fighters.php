...
if($conn){
    ...
    if($res)while($f=$res->fetch_assoc())$all_fighters[$f['id']]=$f;
    ...
    if($rb)while($b=$rb->fetch_assoc()){$rid=(int)($b['red_id']??0);$bid2=(int)($b['blue_id']??0);$e=['event_number'=>$b['event_number'],'bout_type'=>$b['bout_type'],'discipline'=>$b['discipline']??'muay_thai','result_method'=>$b['result_method']??'','result_round'=>$b['result_round']??null,'winner_name'=>$b['winner_name']??null,'red_name'=>$b['red_name']??'','blue_name'=>$b['blue_name']??'','red_gym'=>$b['red_gym']??'','blue_gym'=>$b['blue_gym']??'','red_id'=>$rid,'blue_id'=>$bid2];if($rid)$bh[$rid][]=$e;if($bid2)$bh[$bid2][]=$e;}
}
