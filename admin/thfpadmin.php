...
:root{--fire:#E8341A;--fire-dim:rgba(232,52,26,.15);--fire-border:rgba(232,52,26,.3);--gold:#D4AF37;--black:#080808;--dark:#0d0d0d;--surface:#161616;--white:#fff;--muted:rgba(255,255,255,.5);--dim:rgba(255,255,255,.22);--font-disp:'Cinzel',serif;--font-ui:'Rajdhani',sans-serif;}
...
$conn->query("CREATE TABLE IF NOT EXISTS thfp_events (
    ...
    status ENUM('upcoming','completed','cancelled','live','ongoing') DEFAULT 'upcoming',
...
addColIfMissing($conn,'thfp_fighters','nationality',"VARCHAR(100) DEFAULT '' AFTER gym");
addColIfMissing($conn,'thfp_fighters','fighting_out_of',"VARCHAR(150) DEFAULT '' AFTER nationality");
addColIfMissing($conn,'thfp_fighters','hometown',"VARCHAR(150) DEFAULT '' AFTER fighting_out_of");
addColIfMissing($conn,'thfp_fighters','height',"VARCHAR(50) DEFAULT '' AFTER hometown");
...
addColIfMissing($conn,'thfp_fighters','record_wins',"INT DEFAULT 0 AFTER affiliation");
addColIfMissing($conn,'thfp_fighters','record_losses',"INT DEFAULT 0 AFTER record_wins");
addColIfMissing($conn,'thfp_fighters','record_draws',"INT DEFAULT 0 AFTER record_losses");
addColIfMissing($conn,'thfp_fighters','photo_path',"VARCHAR(255) DEFAULT '' AFTER record_draws");
addColIfMissing($conn,'thfp_fighters','notes',"TEXT DEFAULT NULL AFTER status");
...
addColIfMissing($conn,'thfp_events','officials',"TEXT DEFAULT NULL AFTER sanctioned_by");
addColIfMissing($conn,'thfp_events','sponsors',"TEXT DEFAULT NULL AFTER officials");
addColIfMissing($conn,'thfp_events','production_team',"TEXT DEFAULT NULL AFTER sponsors");
addColIfMissing($conn,'thfp_events','notes',"TEXT DEFAULT NULL AFTER production_team");
...
if (($_POST['action'] ?? '') === 'save_bout') {
    ...
    // ── DISCIPLINE (new) ──
    $disc   = in_array($_POST['discipline'] ?? '', ['muay_thai','kickboxing','mma']) ? $_POST['discipline'] : 'muay_thai';
    $btype  = in_array($_POST['bout_type'] ?? '', ['elimination','final','exhibition','main_event','co_main','prelim','amateur']) ? $_POST['bout_type'] : 'elimination';
    $bord   = (int)($_POST['bout_order'] ?? 1);
    $wc     = sc($_POST['weight_class'] ?? '');
    $gender = in_array($_POST['gender'] ?? '', ['male','female','Male','Female','Mixed','']) ? $_POST['gender'] : '';
    $age    = in_array($_POST['age_category'] ?? '', ['junior','senior','Junior','Senior','Open','']) ? $_POST['age_category'] : '';
    $red    = (int)($_POST['red_fighter_id'] ?? 0) ?: null;
    $blue   = (int)($_POST['blue_fighter_id'] ?? 0) ?: null;
    $winner = (int)($_POST['winner_id'] ?? 0) ?: null;
    $method = in_array($_POST['result_method'] ?? '', ['KO','TKO','RSC','decision','no_contest','SUB','DEC','DRAW','NC','']) ? $_POST['result_method'] : '';
    $dec    = in_array($_POST['decision_type'] ?? '', ['unanimous','split','Unanimous','Split','Majority','']) ? $_POST['decision_type'] : '';
    ...
    if ($id) {
        ...
        $stmt->bind_param("iisssssiiisisssi",$eid,$bnum,$disc,$btype,$bord,$wc,$gender,$age,$red,$blue,$winner,$method,$dec,$round,$time,$notes,$id);
    } else {
        ...
        $stmt->bind_param("iisssssiiississ",$eid,$bnum,$disc,$btype,$bord,$wc,$gender,$age,$red,$blue,$winner,$method,$dec,$round,$time,$notes);
    }
    $stmt->execute(); $stmt->close();
...
.b-exhibition{background:rgba(16,185,129,.1);color:#6ee7b7;border:1px solid rgba(16,185,129,.2);}
...
/* Discipline pill colours in select */
.disc-mt{color:var(--fire-light);}.disc-kb{color:#FF8C00;}.disc-mma{color:#c4b5fd;}
...
<div class="main">
    <div class="topbar">
        ...
        <div class="topbar-actions">
            ...
            <?php elseif($active==='fighters'):?><button class="btn btn-fire" onclick="openModal('mFighter')"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg>Add Fighter</button>
            <?php elseif($active==='bouts'):?><button class="btn btn-fire" onclick="openModal('mBout')"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg>Add Bout</button>
            <?php endif;?>
        </div>
    </div>
...
<?php if($active==='events'):?>
        <div class="panel">
            ...
            <div class="tbl-wrap"><table class="tbl">
                ...
                <tbody>
                <?php while($e=$events->fetch_assoc()):$sc=['completed'=>'b-completed','upcoming'=>'b-upcoming','cancelled'=>'b-cancelled','live'=>'b-upcoming','ongoing'=>'b-upcoming'][$e['status']]??'';?>
                <tr>
                    ...
                    <td><div style="display:flex;gap:.4rem;"><button class="btn btn-outline btn-sm" onclick='editEvent(<?php echo json_encode($e);?>)'>Edit</button><form method="POST" style="display:inline;" onsubmit="return confirm('Delete this event?');"><input type="hidden" name="id" value="<?php echo $e['id'];?>"><button type="submit" name="delete_event" class="btn btn-ghost-danger btn-sm">Del</button></form></div></td>
                </tr>
                <?php endwhile;?>
                </tbody>
            </table></div>
            ...
            <div class="tbl-wrap"><table class="tbl">
                ...
                <tr>
                    <td><div class="f-cell">
                        ...
                        <div><div class="f-name"><?php echo htmlspecialchars($f['name']);?></div><?php if(!empty($f['nickname'])):?><div class="f-meta">&ldquo;<?php echo htmlspecialchars($f['nickname']);?>&rdquo;</div><?php endif;?></div>
                    </div></td>
                    ...
                    <td><div style="display:flex;gap:.4rem;"><button class="btn btn-outline btn-sm" onclick='editFighter(<?php echo json_encode($f);?>)'>Edit</button><form method="POST" style="display:inline;" onsubmit="return confirm('Delete?');"><input type="hidden" name="id" value="<?php echo $f['id'];?>"><button type="submit" name="delete_fighter" class="btn btn-ghost-danger btn-sm">Del</button></form></div></td>
                </tr>
                <?php endwhile;?>
                </tbody>
            </table></div>
        ...
        <div class="panel">
            ...
            <div class="tbl-wrap"><table class="tbl">
                ...
                <?php while($b=$bouts->fetch_assoc()):
                    ...
                    $tb=['elimination'=>'b-elimination','final'=>'b-final','exhibition'=>'b-exhibition','main_event'=>'b-main_event','prelim'=>'b-prelim'][$b['bout_type']]??'b-elimination';
                    $db='b-'.($b['discipline']??'muay_thai');
                    $mb=['KO'=>'b-ko','TKO'=>'b-tko','RSC'=>'b-rsc','decision'=>'b-decision','no_contest'=>'b-no_contest'][$b['result_method']]??'';
                ?>
                <tr>
                    ...
                    <td><span class="badge <?php echo $db;?>"><?php echo $disc_labels[$b['discipline']??'muay_thai']??'Muay Thai';?></span></td>
                    ...
                    <td><?php if($b['result_method']):?><span class="badge <?php echo $mb;?>"><?php echo strtoupper($b['result_method']);?></span><?php if($b['decision_type']):?><span style="font-size:.7rem;color:var(--dim);display:block;"><?php echo ucfirst($b['decision_type']);?></span><?php endif;?><?php if($b['result_round']):?><span style="font-size:.7rem;color:var(--dim);">R<?php echo $b['result_round'];?></span><?php endif;?><?php else:?><span style="color:var(--dim);">—</span><?php endif;?></td>
                    <td style="font-size:.84rem;font-weight:600;color:var(--fire-light);"><?php echo htmlspecialchars($b['winner_name']??'—');?></td>
                    <td><div style="display:flex;gap:.4rem;"><button class="btn btn-outline btn-sm" onclick='editBout(<?php echo json_encode($b);?>)'>Edit</button><form method="POST" style="display:inline;" onsubmit="return confirm('Delete?');"><input type="hidden" name="id" value="<?php echo $b['id'];?>"><button type="submit" name="delete_bout" class="btn btn-ghost-danger btn-sm">Del</button></form></div></td>
                </tr>
                <?php endwhile;?>
                </tbody>
            </table></div>
...
<div class="modal">
    ...
    <form method="POST">
        ...
        <div class="modal-body"><div class="grid2">
            <div class="fg full"><label>Event Name *</label><input type="text" name="name" id="ev_name" required placeholder="e.g. Tribal Hunters – Combat2"></div>
            <div class="fg"><label>Event Number</label><input type="number" name="event_number" id="ev_num" min="1" value="1"></div>
            <div class="fg"><label>Status</label><select name="status" id="ev_status"><option value="upcoming">Upcoming</option><option value="completed">Completed</option><option value="ongoing">Ongoing</option><option value="live">Live</option><option value="cancelled">Cancelled</option></select></div>
            <div class="fg"><label>Date</label><input type="date" name="event_date" id="ev_date"></div>
            <div class="fg"><label>Venue</label><input type="text" name="venue" id="ev_venue"></div>
        ...
        <div class="modal-body">
            <div class="fg full" style="margin-bottom:1.2rem;">
                ...
                <div class="photo-upload-area">
                    <div id="photoPlaceholder"><svg width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="var(--fire)" stroke-width="1.5" style="margin:0 auto .5rem;display:block;"><path d="M21 15v4a2 2 0 01-2 2H5a2 2 0 01-2-2v-4"/><polyline points="17 8 12 3 7 8"/><line x1="12" y1="3" x2="12" y2="15"/></svg><div style="font-size:.8rem;font-weight:600;">Click to upload photo</div></div>
            ...
            <div class="grid2">
                ...
                <div class="fg"><label>Date of Birth</label><input type="date" name="date_of_birth" id="fi_dob"></div>
                ...
                <div class="fg full"><label>Primary Gym / Team</label><input type="text" name="gym" id="fi_gym"></div>
                <div class="fg full"><label>Affiliation / Camp</label><input type="text" name="affiliation" id="fi_affil"></div>
                <div class="form-sep">Competition Details</div>
                <div class="fg"><label>Weight Class</label><select name="weight_class" id="fi_wc"><option value="">— Select —</option><option>48</option><option>51</option><option>54</option><option>57</option><option>60</option><option>63.5</option><option>67</option><option>70</option><option>75</option><option>81</option><option>91</option><option>105</option></select></div>
                ...
                <div class="fg"><label>Gender</label><select name="gender" id="fi_gender"><option value="">— Select —</option><option value="Male">Male</option><option value="Female">Female</option></select></div>
                <div class="fg"><label>Age Category</label><select name="age_category" id="fi_age"><option value="">— Select —</option><option value="Junior">Junior</option><option value="Senior">Senior</option></select></div>
                <div class="form-sep">Location</div>
                <div class="fg"><label>Hometown</label><input type="text" name="hometown" id="fi_town"></div>
                ...
                <div class="fg"><label>Status</label><select name="status" id="fi_st"><option value="active">Active</option><option value="inactive">Inactive</option><option value="retired">Retired</option></select></div>
                <div class="fg full"><label>Notes</label><textarea name="notes" id="fi_notes"></textarea></div>
            </div>
        </div>
...
function previewPhoto(input){if(input.files&&input.files[0]){var reader=new FileReader();reader.onload=function(e){var pw=document.getElementById('photoPreviewWrap');var pi=document.getElementById('photoPreviewImg');pi.src=e.target.result;pw.style.display='flex';};reader.readAsDataURL(input.files[0]);}}
