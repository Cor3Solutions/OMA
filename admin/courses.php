...
function buildPaginationBar($total, $per_page, $current_page, $extra_params = [])
{
    ...
    ob_start(); ?>
    <div
        style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:1rem;margin-top:1rem;padding:.8rem 1rem;background:#f8f9fa;border-radius:8px;border:1px solid #e9ecef;">
        <div style="display:flex;align-items:center;gap:1rem;flex-wrap:wrap;">
            <span style="color:#666;font-size:.88rem;">
                Showing
                <strong><?= min(($current_page - 1) * $per_page + 1, $total) ?>–<?= min($current_page * $per_page, $total) ?></strong>
                of <strong><?= $total ?></strong>
            </span>
            <form method="GET" style="display:flex;align-items:center;gap:.4rem;">
                <?php foreach ($extra_params as $k => $v): ?><input type="hidden" name="<?= $k ?>"
                        value="<?= htmlspecialchars($v) ?>"><?php endforeach; ?>
                <input type="hidden" name="page" value="1">
                <label style="font-size:.85rem;color:#666;">Rows:</label>
                <select name="per_page" onchange="this.form.submit()"
                    style="padding:.3rem .5rem;border:1px solid #ddd;border-radius:5px;font-size:.85rem;cursor:pointer;">
        ...
        <?php if ($total_pages > 1): ?>
            <div style="display:flex;gap:.3rem;align-items:center;">
                ...
                $rng = 2;
                $sp = max(1, $current_page - $rng);
                $ep = min($total_pages, $current_page + $rng);
                if ($ep - $sp < $rng * 2) {
                    $sp = max(1, $ep - $rng * 2);
                    $ep = min($total_pages, $sp + $rng * 2);
                }
                if ($sp > 1) {
                    echo "<a href='{$makeUrl(1)}' style='{$btnBase}'>1</a>";
                    if ($sp > 2)
                        echo "<span style='padding:.35rem .5rem;color:#999;font-size:.85rem;'>…</span>";
                }
                for ($p = $sp; $p <= $ep; $p++) {
                    $a = $p === $current_page ? $btnActive : '';
                    echo "<a href='{$makeUrl($p)}' style='{$btnBase}{$a}'>{$p}</a>";
                }
                if ($ep < $total_pages) {
                    if ($ep < $total_pages - 1)
                        echo "<span style='padding:.35rem .5rem;color:#999;font-size:.85rem;'>…</span>";
                    echo "<a href='{$makeUrl($total_pages)}' style='{$btnBase}'>{$total_pages}</a>";
                }
        ...
        if (empty($error)) {
            if ($is_update) {
                ...
                logActivity(
                    ...
                    $course_id,
                    $title,
                    'Course material updated. Status: ' . $status
                );
            } else {
                ...
                logActivity(
                    ...
                    $conn->insert_id,
                    $title,
                    'Course material created. Category: ' . $category .
...
if ($_cur_page > max(1, ceil($_total_courses / $_per_page))) {
    $_cur_page = max(1, ceil($_total_courses / $_per_page));
    $_offset = ($_cur_page - 1) * $_per_page;
}
                    ...
                    <tr style="<?php echo $course['status'] === 'archived' ? 'opacity:0.6; background:#f9f9f9;' : ''; ?>">
                        ...
                        <td>
                            <?php if ($course['khan_level_min'] == $course['khan_level_max']): ?>
                                <span class="badge" style="background:#2196F3; color:white;">Khan
                                    <?php echo $course['khan_level_min']; ?></span>
                            <?php else: ?>
                                <span class="badge" style="background:#673AB7; color:white;">Khan
                        ...
                        <td><span class="badge"
                                style="background: <?php echo $course['is_public'] ? '#388e3c' : '#f57c00'; ?>; color: white;"><?php echo $course['is_public'] ? 'Public' : 'Members Only'; ?></span>
                        </td>
                        <td>
                            <span
                                class="badge badge-<?php echo $course['status'] === 'published' ? 'active' : 'inactive'; ?>">
                        ...
                        <td>
                            <div style="display: flex; gap: 5px;">
                                ...
                                <?php if ($course['status'] !== 'archived'): ?>
                                    <form method="POST"
                                        onsubmit="return confirm('Archive this course? It will be hidden from users.');">
                                        <input type="hidden" name="id" value="<?php echo $course['id']; ?>">
                                        <button type="submit" name="archive_course" class="btn btn-sm btn-warning"
                                            title="Archive">
    ...
    <div class="modal-content">
        ...
        <form method="POST" enctype="multipart/form-data" id="courseForm">
            ...
            <div class="form-grid">
                ...
                <div class="form-group"
                    style="border: 1px solid #ddd; padding: 10px; border-radius: 4px; background: #fafafa;">
                    <label class="form-label">Target Khan Level</label>
                    <label
                        style="display: flex; align-items: center; gap: 8px; margin-bottom: 8px; font-size: 0.9em; cursor: pointer;">
                        <input type="checkbox" name="use_range_toggle" id="use_range_toggle"
                            onchange="toggleRangeInputs()">
                    ...
                    <div id="single_level_input">
                        <select name="khan_single" id="khan_single" class="form-select">
                            <?php for ($i = 1; $i <= 16; $i++):
                                echo "<option value='$i'>Khan $i</option>";
                            endfor; ?>
                        </select>
                    </div>
                    ...
                    <div id="range_level_input" style="display:none; gap: 0.5rem; align-items: center;">
                        <select name="khan_min" id="khan_min" class="form-select" style="flex:1;">
                            <?php for ($i = 1; $i <= 16; $i++):
                                echo "<option value='$i'>From Khan $i</option>";
                            endfor; ?>
                        </select>
                        <span>to</span>
                        <select name="khan_max" id="khan_max" class="form-select" style="flex:1;">
                            <?php for ($i = 1; $i <= 16; $i++):
                                echo "<option value='$i' " . ($i == 16 ? 'selected' : '') . ">To Khan $i</option>";
                            endfor; ?>
                        </select>
                    </div>
                </div>
            ...
            <div class="form-group"
                style="background: #f0f7ff; padding: 15px; border-radius: 8px; border: 1px solid #cce5ff;">
